...
class AVHubertModel(PreTrainedModel):
    ...
    def forward_gen(
        ...
        if isinstance(features, dict):
             orig_features = features['features']
             features.pop('features')
        ...
        if features_only:
            if remaining_features is None:
                ...
                #print(remaining_features.keys())
                return {"x": x, "padding_mask": padding_mask, "features": features, "remaining_features": remaining_features}
        ...
        if 'remaining_features' is not None:
            result['remaining_features'] = remaining_features
    ...
    def forward(
        ...
        **kwargs,
        ...
        feature = res["x"]
        latents = res["remaining_features"]
        #latents=None
        # Custom
        if 'remaining_features' in res:
            #return BaseModelOutput(last_hidden_state=feature, hidden_states=None, attentions=None), res['remaining_features']
            return BaseModelOutputWithLatents(last_hidden_state=feature, hidden_states=None, attentions=None, latents=latents)
